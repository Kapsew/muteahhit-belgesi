...
Madde haritası:
  ...
  67. madde → yol kotu altı kat sayısı (sayı — opsiyonel)
  68. madde → yol kotu üstü kat sayısı (sayı — opsiyonel)
  69. madde → toplam kat sayısı (sayı — opsiyonel)
  73. madde → toplam yapı yüksekliği (m, sayı olarak — opsiyonel)
...
Tarihler gg.aa.yyyy formatında. Sayıların Türkçe binlik ayracını (.) temizle.`;
      ...
      properties: {
        ...
        is: {
          ...
          properties: {
            ...
            yapiYuksekligi: { type: ["number","null"], description: "toplam yapı yüksekliği m (73. madde)" },
  ...
  for (const d of dosyalar) {
    ...
    if (mime === "application/pdf") {
      ...
      const sayfa = pdfSayfaSayisi(d.base64);
      if (sayfa === 0) {
...
function pdfSayfaSayisi(base64: string): number {
  try {
    const bin = atob(base64);
    const eslesme = bin.match(/\/Type\s*\/Page[^s]/g);
    const sayi = eslesme ? eslesme.length : 0;
    return sayi > 0 ? sayi : 1;
  } catch (e: any) {
